...
def build_delivery_monitor(repo: OrderRepository) -> dict[str, Any]:
    ...
    for courier in COURIERS:
        ...
        courier_cards.append({
            ...
            "new_orders": sum(
                order.status == "pending" and not order.courier_read_at for order in active
            ),
            "waiting_pickup": sum(
                order.status == "pending" and bool(order.courier_read_at) for order in active
            ),
    ...
    if unassigned_stops:
        routes.append({
            ...
            "heading_to_warehouse": False,
            "warehouse_started_at": None,
    ...
    return {
        ...
        "summary": {
            ...
            "new_orders": sum(
                order.status == "pending" and not order.courier_read_at
                for order in active_orders
            ),
            "waiting_pickup": sum(
                order.status == "pending" and bool(order.courier_read_at)
                for order in active_orders
            ),
