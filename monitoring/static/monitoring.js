...
function renderOverview(payload){
  ...
  blocks.push(delivery.data?panel("Доставка","Полный мониторинг и календарь",`<div class="metric-grid">${metric(number(ds.active),"Активные")}${metric(number(ds.on_way),"В пути")}${metric(number(ds.completed_today),"Завершено")}${metric(number(ds.attention),"Внимание","danger")}</div>`,"ok","/delivery/monitor"):unavailable("Доставка"));
