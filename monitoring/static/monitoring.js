...
async function renderPrices(){const payload=await api("/monitoring/api/prices"),data=payload.data||{},snapshot=data.snapshot||{};$("content").innerHTML=`<section class="metric-grid">${metric(data.status||"—","Статус")}${metric(number(data.sections?.length),"Разделов")}${metric(number(snapshot.product_count),"Товаров")}${metric(data.scheduler_running?"Работает":"Остановлен","Планировщик")}</section><section class="section-grid">${panel("Разделы прайса","Только просмотр для менеджеров",table(["Раздел","Товаров","Изменения"],(data.sections||[]).map(row=>`<tr><td><b>${esc(row.title)}</b><br><span class="muted">${esc(row.section_key)}</span></td><td>${number(row.product_count)}</td><td>${row.changed_recent?"Есть":"Нет"}</td></tr>`)))}${panel("Безопасный режим","Публикация ограничена",`<p class="muted">Менеджеры просматривают полный прайс после единого входа. Изменение и публикация цен остаются в отдельном административном контуре.</p>`)}</section><section class="catalog-panel"><div class="panel-head"><div><h2>Полный прайс</h2><p>Актуальный каталог без повторного пароля</p></div><a class="panel-link catalog-open" href="/monitoring/prices/catalog" target="_blank" rel="noopener">Открыть отдельно ↗</a></div><iframe class="price-catalog" title="Полный прайс TEXNIKACH" src="/monitoring/prices/catalog" sandbox="allow-scripts allow-popups allow-popups-to-escape-sandbox"></iframe></section>`}
