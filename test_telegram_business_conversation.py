...
class Harness:
    ...
    def run_location_phone(self):
        actions = [
            row
            for row in self.service.repo.due_actions(self.clock())
            if row["action_type"] == "location_phone"
        ]
        assert actions
        for action in actions:
            ...
            self.service.execute(action)
            self.service.repo.finish_action(
                action["action_id"], self.clock(), generation=action["generation"]
            )
...
def test_telegram_location_before_model_is_saved_and_acknowledged(tmp_path):
    ...
    harness.run_location_phone()
    harness.run_debounce()
    saved = harness.service.repo.session_by_id(session["session_id"])
    assert saved["location_received"] == 1
    assert saved["location_url"] == "https://www.google.com/maps?q=41.311081,69.240562"
    assert len(harness.api.sent) == 2
    assert harness.api.sent[0]["text"] == (
        "Можете отправить свой номер телефона?\n"
        "Отправьте его через 📎 → Контакт."
    )
    assert "Локацию получили" in harness.api.sent[1]["text"]
    assert harness.products.queries == []


def test_location_phone_request_runs_in_daytime_and_under_manager_lock(tmp_path):
    harness = Harness(tmp_path, datetime(2026, 8, 23, 12, 0, tzinfo=TZ))
    session = harness.incoming(harness.clock(), "Здравствуйте", chat_id=220)
    harness.service.repo.manager_answer(
        "220",
        harness.clock(),
        120,
        event_at=harness.clock(),
    ...
    harness.run_location_phone()
    assert [row["text"] for row in harness.api.sent] == [
        "Можете отправить свой номер телефона?\n"
        "Отправьте его через 📎 → Контакт."
    ]


def test_manager_reply_does_not_cancel_queued_location_phone_action(tmp_path):
    harness = Harness(tmp_path, datetime(2026, 8, 23, 12, 0, tzinfo=TZ))
    harness.incoming(
        harness.clock(),
        chat_id=2201,
    ...
    manager_update = {
        ...
        "business_message": {
            ...
            "chat": {"id": 2201, "type": "private"},
            "from": {"id": 100, "language_code": "ru"},
            "text": "Хорошо",
        },
    }
    assert harness.service.repo.save_update(manager_update, harness.clock())
    harness.service.process_update(manager_update)
    harness.run_location_phone()
    assert [row["text"] for row in harness.api.sent] == [
        "Можете отправить свой номер телефона?\n"
        "Отправьте его через 📎 → Контакт."
...
def test_location_phone_flow_respects_permanent_bot_pause(tmp_path):
    harness = Harness(tmp_path, datetime(2026, 8, 23, 12, 0, tzinfo=TZ))
    harness.incoming(harness.clock(), "Здравствуйте", chat_id=221)
    assert harness.service.repo.set_bot_paused(
        "221", True, harness.clock(), "excluded_client"
    )
    harness.incoming(
        harness.clock(),
        chat_id=221,
    ...
    assert harness.api.sent == []
    assert not harness.service.repo.location_phone_pending("221")


def test_verified_own_contact_is_confirmed_with_two_messages_after_location(tmp_path):
    harness = Harness(tmp_path, datetime(2026, 8, 23, 12, 0, tzinfo=TZ))
    harness.incoming(
        harness.clock(),
        chat_id=222,
    ...
    assert harness.service.repo.verified_client_phone("222") == "+998901234567"
    assert not harness.service.repo.location_phone_pending("222")


def test_unknown_language_does_not_repeat_phone_number_in_bilingual_reply(tmp_path):
    harness = Harness(tmp_path, datetime(2026, 8, 23, 12, 0, tzinfo=TZ))
    harness.incoming(
        harness.clock(),
        chat_id=2221,
    ...
    harness.run_location_phone()
    assert harness.api.sent[0]["text"] == "+998901234567"
    assert harness.api.sent[1]["text"] == "Ваш номер\n\n———\n\nSizi nomeriz"


def test_pending_location_accepts_only_contact_owned_by_sender(tmp_path):
    harness = Harness(tmp_path, datetime(2026, 8, 23, 12, 0, tzinfo=TZ))
    harness.incoming(
        harness.clock(),
        chat_id=223,
    ...
    harness.run_location_phone()
    assert harness.service.repo.location_phone_pending("223")
    ...
    harness.run_location_phone()
    assert harness.service.repo.verified_client_phone("223") is None
    assert harness.service.repo.location_phone_pending("223")
    assert [row["text"] for row in harness.api.sent].count(
        "Можете отправить свой номер телефона?\n"
        "Отправьте его через 📎 → Контакт."
    ) == 2
    ...
    harness.run_location_phone()
    assert [row["text"] for row in harness.api.sent][-2:] == [
        "+998901234567",
        "Ваш номер",
    ]
    assert harness.service.repo.verified_client_phone("223") == "+998901234567"
    assert not harness.service.repo.location_phone_pending("223")


def test_location_phone_delivery_is_idempotent_for_repeated_business_message(tmp_path):
    harness = Harness(tmp_path, datetime(2026, 8, 23, 12, 0, tzinfo=TZ))
    harness.incoming(
        harness.clock(),
        chat_id=224,
    ...
    duplicate = {
        ...
        "business_message": {
            ...
            "message_id": harness.update_id,
            "date": int(harness.clock().timestamp()),
            "chat": {"id": 224, "type": "private"},
            "from": {"id": 224, "language_code": "ru"},
            "location": {"latitude": 41.311081, "longitude": 69.240562},
        },
    }
    harness.service.process_update(duplicate)
    assert len(harness.api.sent) == 1
