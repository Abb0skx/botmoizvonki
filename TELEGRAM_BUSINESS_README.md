...
После каждой новой локации бот отдельно проверяет подтверждённый номер клиента.
Эта короткая проверка работает круглосуточно и не блокируется ручным ответом
менеджера; постоянный `bot_paused` по-прежнему имеет приоритет. Номер считается
принадлежащим клиенту только для нативного Telegram Contact, у которого
`contact.user_id` совпадает с `message.from.id`. Введённый текстом номер, чужой
контакт и контакт без `user_id` для этой проверки не подходят. Если своего
контакта нет, бот просит отправить его через **📎 → Контакт**. Telegram Bot API
не поддерживает `ReplyKeyboardMarkup/request_contact` для сообщений от имени
Business-аккаунта, поэтому отдельной системной кнопки запроса контакта здесь нет.
