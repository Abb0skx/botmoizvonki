...
class EntryRouteTests(unittest.TestCase):
    ...
    def test_shell_has_no_data_and_strict_csp(self):
        ...
        self.assertIn('price-entry.css?v=11', response.text)
        self.assertIn('price-entry.js?v=11', response.text)
        self.assertIn('id="save-hint"', response.text)
    ...
    def test_public_assets_whitelisted(self):
        ...
        self.assertIn('function invalidEdits()', entry_js.text)
        self.assertIn('input.dataset.editKey = key(row, field)', entry_js.text)
        self.assertIn('Нажмите кнопку — покажем ячейку, которую нужно исправить', entry_js.text)
        self.assertIn('focusEdit(first);', entry_js.text)
        self.assertNotIn('state.edits.size > 200 || [...state.edits.values()].some', entry_js.text)
