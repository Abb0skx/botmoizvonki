(() => {
  ...
  function openEditor(initial = {}, draftId = null) {
    ...
    const save = button(editing ? "Сохранить модель" : "Создать модель", async () => {
      ...
      if (!model.value.trim() || !category.value || !rows.length || rows.length > 100 || (category.value === "__new__" && !newCategory.value.trim())) { status.textContent = "Заполните модель, категорию и от 1 до 100 вариантов."; status.classList.add("danger"); return; }
